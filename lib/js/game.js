...
function preload() {
  ...
  this.load.tilemapTiledJSON("map", "assets/tiles/scripts/tile.json");
  this.load.image("player", "assets/units/player.png");
}
...
function create() {
  ...
  // var coinTiles = map.addTilesetImage("map", "coin", 32, 32, 1, 2);

  // game.add.tileSprite(300, 450, 200, 100, "starfield");
  // sprite = this.add.tileSprite(0, 0, 126, 300, "coin");
  // sprite.animations.add("rotate");
  // sprite.animations.play("rotate", 20, true);

  var coinTiles = map.addTilesetImage("coin");
  map.createLayer("Background Layer", groundTiles, 0, 0);
  groundLayer = map.createLayer("Ground Layer", groundTiles, 0, 0);
  // coinLayer = map.createLayer("Coin Layer", coinTiles);
